...
In addition to the command-line interface, Pubmedsoso also provides a graphical user interface. You can launch it by running:
...
The GUI provides an intuitive interface for setting search keywords, page numbers, download counts, year range, and other parameters. It also displays real-time log output and progress during execution.
...
* [x] A usable GUI interface (implemented with tkinter)
...
2026.03.31 Added GUI interface based on tkinter, added file selection UI, improved logging system, and enhanced PDF download functionality.
