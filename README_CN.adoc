...
除了命令行方式，Pubmedsoso 还提供了图形用户界面。运行以下命令启动：
...
GUI 提供了直观的界面，可以设置搜索关键词、页数、下载数量、年份范围等参数，并且在执行过程中实时显示日志输出和进度。
...
== Update log ==
    ...
    2026.03.31 新增基于 tkinter 的 GUI 图形界面，新增文件选择UI，完善日志系统，增强 PDF 下载功能。